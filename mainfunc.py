...
helmet_classifier_weight = r'weights/helment_no_helmet98.6.pth'


# yolov5_weight_file = urllib.request.urlopen('https://drive.google.com/uc?export=download&id=1H5OnB1wE9Uv79CXr0n58o_LyjXGew0sp')
# helmet_classifier_weight = urllib.request.urlopen('https://drive.google.com/uc?export=download&id=1roCeuyqNyp1eSoZx_3o6xpBhzEnVLIEi')

# yolov5_weight_file.read()
# helmet_classifier_weight.read()


# # import urllib.request

# # MODEL_URL = "https://drive.google.com/uc?export=download&id=YOUR_FILE_ID"
# urllib.request.urlretrieve(yolov5_weight_file, "rider_helmet_number_medium.pt")
# urllib.request.urlretrieve(helmet_classifier_weight, "helment_no_helmet98.6.pth")


# model = load_learner("", "rider_helmet_number_medium.pt")
# model2 = load_learner("", "helment_no_helmet98.6.pth")



# yolov5_weight_file = urllib.request.urlopen('https://drive.google.com/file/d/1H5OnB1wE9Uv79CXr0n58o_LyjXGew0sp/view?usp=sharing')
# helmet_classifier_weight = urllib.request.urlopen('https://drive.google.com/file/d/1roCeuyqNyp1eSoZx_3o6xpBhzEnVLIEi/view?usp=sharing')
...
conf_set=0.3
frame_size=(800, 480)
head_classification_threshold= 0.8 # make this value lower if want to detect non helmet more aggresively;
...
def img_classify(frame):
	print(frame.shape," CHECK THIS")
	print('Head size: ',frame.shape[:-1])

	if frame.shape[0]<50: # skiping small size heads <----------------  you can adjust this value
		return [None, 0]

	frame = transform(Image.fromarray(frame))
	frame = frame.unsqueeze(0)
	prediction = model2(frame)
	result_idx = torch.argmax(prediction).item()   # what is it, helmet or not wearing helmet
	prediction_conf = sorted(prediction[0]) #take only the required part of prediction and return them in ascending order
	print(prediction, "this is helmet prediction")
	print(prediction_conf,"this is prediction confidence")
